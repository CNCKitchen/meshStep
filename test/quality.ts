...
for(const f of ["cylinderWithHole","roundedCube","MicHolder_Inserts","everything","splineThing"]){
  const src=readFileSync(join(root,f+".step"),"utf8");
  const noR=importStep(src,{remesh:false,surfaceDeviation:0.02,maxEdge:1});
  const t0=Date.now();
  const reM=importStep(src,{remesh:true,surfaceDeviation:0.02,maxEdge:1});
  console.log(`${f}: NO-REMESH ${JSON.stringify(stats(noR.mesh))}  ->  REMESH(${Date.now()-t0}ms) ${JSON.stringify(stats(reM.mesh))}`);
}
