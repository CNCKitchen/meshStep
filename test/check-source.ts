...
function stats(res: any) {
  const m = res.mesh;
  // Open-shell surface bodies have open boundaries BY DESIGN — exclude them from the count.
  const openSet = new Set<number>(res.openSolids ?? []);
  const I = m.indices, P = m.positions, nt = I.length / 3, und = new Map<string, number>();
  let sl = 0;
  for (let t = 0; t < nt; t++) {
    if (!openSet.has(res.solidOfTri[t])) {
...
for (const f of readdirSync(dir).filter((x) => x.toLowerCase().endsWith(".step") || x.toLowerCase().endsWith(".stp"))) {
  ...
  try {
    const s = stats(importStep(src, { remesh: false, surfaceDeviation: 0.002, maxEdge: 1 }));
